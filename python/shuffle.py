...
class Card:  # card object with a value and suit, i.e (3,hearts)
    def __init__(self, value, suit):
        match value:  # switch statement for card generation
            ...
            case 13:
                self.value = "King"
            case _:  # default for non-face card value
                self.value = value
        self.suit = suit
...
def non_perfect_riffle_shuffle(deck):
    ...
    deck1 = deck[: len(deck) // 2]  # list splicing
...
def perfect_riffle_shuffle(deck):
    ...
    deck, second_deck = (
        deck[:middle_point],
        deck[middle_point:],
    )  # first card up to middle point, middle to the last card
...
if __name__ == "__main__":
    ...
    ]  # list comp to generate a list of Card objects with a range of values
...
class RiffleTestCase(unittest.TestCase):
    def test_perfect_riffle_shuffle(self):
        ...
        """
        riffleEight = perfect_riffle_shuffle(
                ...
                perfect_riffle_shuffle(
                    perfect_riffle_shuffle(
                        perfect_riffle_shuffle(
                            perfect_riffle_shuffle(
                                perfect_riffle_shuffle(
                                    perfect_riffle_shuffle(ordered_deck)
                                )
                            )
                        )
                    )
